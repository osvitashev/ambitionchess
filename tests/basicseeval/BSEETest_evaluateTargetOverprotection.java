...
class BSEETest_evaluateTargetOverprotection {
	...
	void test(int sq, int player) {
		test_eval.evaluateTargetPotection(sq, player);
	}
	...
	void test() {
		...
		test(Square.F5, Player.WHITE);
		test(Square.A6, Player.WHITE);
		test_game.loadFromFEN("3kr3/6b1/8/4p3/8/3N4/2K5/8 w - - 0 1");
		test_eval.initialize();
		test(Square.E5, Player.WHITE);
