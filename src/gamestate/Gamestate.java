...
//	}
	...
	public Gamestate loadFromFEN(String fen) {
		...
		if(getPieces(Player.WHITE, PieceType.KING) == 0)
			...
			kingSquare[0] = Bitboard.getFirstSquareIndex(getPieces(Player.WHITE, PieceType.KING));
		
		if(getPieces(Player.BLACK, PieceType.KING) == 0)
			throw new RuntimeException("Invalid Game State!");
		else
			kingSquare[1] = Bitboard.getFirstSquareIndex(getPieces(Player.BLACK, PieceType.KING));
